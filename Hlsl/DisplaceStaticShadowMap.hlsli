// -----------------------------------------------------------------------
// 
// スタティックメッシュ　カスケードシャドウマッピング　ディスプレースメントマッピングのシェーダー
//                                                              2021.6.11
// （フォグの処理が入る）
//                                           DisplaceStaticShadowMap.hlsli
// -----------------------------------------------------------------------
...
Texture2D g_HeightTexture: register(t2);
...
float4 PS(DS_OUTPUT In) : SV_Target
{
	...
	if (width == 0)  // ディフューズテクスチャがないとき  // -- 2020.12.15
	{
		...
		texColor = g_Texture.Sample(g_samLinear, In.uv);  // ディフューズテクスチャ
		texSpecular = g_SpecularTexture.SampleLevel(g_samLinear, In.uv, 0);  // スペキュラテクスチャ
	}
	float4 texNormal = g_NormalTexture.SampleLevel(g_samLinear, In.uv, 0);  // ノーマルマップ
	...
	if (g_LightDir.x == 0 && g_LightDir.y == 0 && g_LightDir.z == 0)
	{
		...
		if (texNormal.w == 1.0f)   // 法線マップがあるかどうかチェック
		{
			...
			// 法線マッピングの明るさ計算
			float3 normalVector = -normalize(2.0 * texNormal.xyz - 1.0f);  // ベクトルへ変換(法線マップの色は0～1.0。これを-1.0～1.0のベクトル範囲にする)
			float  NMbright = max(0, dot(normalVector, g_LightDir.xyz));  // ノーマルマップの光計算

			// 光源の方向計算
			float halflamb = max(0, dot(In.Normal, g_LightDir.xyz));  // ランバートの余弦則
