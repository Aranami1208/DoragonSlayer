...
namespace {
	...
	struct DrawObject {
		GameObject* object;
		int order;
		float distQ;	  // éãì_Ç©ÇÁÇÃãóó£ÇÃÇQèÊ
		bool visible;
	...
	bool needSortUpdate;
	bool needSortDraw;
	int  drawTimes = 1;      // ï`âÊâÒêîÅièâä˙ílÇÕ1âÒÅj
	int  drawCounter;        // ï`âÊÉJÉEÉìÉ^Å[ÅiâΩâÒñ⁄ÇÃï`âÊÇ©Åj
};
...
void ObjectManager::Draw()
{
	...
	{
		if (needSortDraw) {
			// É\Å[ÉgèàóùÅ@ÉLÅ[ÇP:óDêÊìxÇÃç~èáÅ@ÉLÅ[ÇQ:éãì_Ç©ÇÁÇÃãóó£ÇÃç~èá
