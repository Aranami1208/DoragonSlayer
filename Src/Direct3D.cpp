...
HRESULT CDirect3D::InitD3D(HWND hWnd, DWORD dwWidth, DWORD dwHeight)
{
	...
	// ShadowMapテクスチャの大きさ
	// ・DirectX11の最大テクスチャサイズ16384
	m_dwDepthTexWidth = 5120;    // -- 2020.3.31
	m_dwDepthTexHeight = 5120;

	// カスケードシャドウマップ用に、カスケード段数分のテクスチャを作成する  // -- 2020.4.5
	for (int i = 0; i < MAX_CASCADE_SHADOW; i++)
	{
		// 深度マップテクスチャーを作成
		D3D11_TEXTURE2D_DESC tdesc;
		ZeroMemory(&tdesc, sizeof(D3D11_TEXTURE2D_DESC));
		tdesc.Width = m_dwDepthTexWidth;
		tdesc.Height = m_dwDepthTexHeight;
		tdesc.MipLevels = 1;
		tdesc.ArraySize = 1;
		tdesc.MiscFlags = 0;
		...
		m_pDevice->CreateTexture2D(&tdesc, nullptr, &m_pDepthMap_Tex[i]);
		...
		m_pDevice->CreateRenderTargetView(m_pDepthMap_Tex[i], &DescRT, &m_pDepthMap_TexRTV[i]);

		// 深度マップテクスチャをレンダーターゲットにする際のデプスステンシルビュー用のテクスチャーを作成
		descDepth.Width = m_dwDepthTexWidth;
		descDepth.Height = m_dwDepthTexHeight;
		descDepth.MipLevels = 1;
		descDepth.ArraySize = 1;
		descDepth.Format = DXGI_FORMAT_D32_FLOAT;
		...
		m_pDevice->CreateTexture2D(&descDepth, nullptr, &m_pDepthMap_DSTex[i]);

		// そのテクスチャーに対しデプスステンシルビュー(DSV)を作成
		m_pDevice->CreateDepthStencilView(m_pDepthMap_DSTex[i], nullptr, &m_pDepthMap_DSTexDSV[i]);
		...
		m_pDevice->CreateShaderResourceView(m_pDepthMap_Tex[i], &SRVDesc, &m_pDepthMap_TexSRV[i]);

		// ビューを作ったらテクスチャは不要なので解放する            // -- 2022.6.24
		SAFE_RELEASE(m_pDepthMap_Tex[i]);
		SAFE_RELEASE(m_pDepthMap_DSTex[i]);
...
HRESULT CDirect3D::InitSampler()
{
	...
	// ----------------------------------------------------------------------
	//テクスチャー用サンプラー作成　(SamplerComparisionState)
	// (シャドウマップでエッジぼかし用に使用する)
	ZeroMemory(&SamDesc, sizeof(D3D11_SAMPLER_DESC));

	SamDesc.AddressU = D3D11_TEXTURE_ADDRESS_BORDER;
	SamDesc.AddressV = D3D11_TEXTURE_ADDRESS_BORDER;
	SamDesc.AddressW = D3D11_TEXTURE_ADDRESS_BORDER;
	VECTOR4 colorB = VECTOR4(1.0f, 1.0f, 1.0f, 1.0f);
	memcpy_s(SamDesc.BorderColor, sizeof(SamDesc.BorderColor), &colorB, sizeof(VECTOR4)); // Addressにて_BORDERが指定されている場合に使用する境界色 (範囲: 0.0~1.0)

	SamDesc.ComparisonFunc = D3D11_COMPARISON_LESS_EQUAL;  // 比較オプション。

	SamDesc.Filter = D3D11_FILTER_COMPARISON_MIN_MAG_MIP_LINEAR;  // 比較結果をバイリニア補完
	...
	m_pDevice->CreateSamplerState(&SamDesc, &m_pSampleComp);
...
void CDirect3D::DestroyD3D()
{
	...
	{
		SAFE_RELEASE(m_pDepthMap_Tex[i]);
		SAFE_RELEASE(m_pDepthMap_TexRTV[i]);
		SAFE_RELEASE(m_pDepthMap_DSTex[i]);
		SAFE_RELEASE(m_pDepthMap_DSTexRTV[i]);
		SAFE_RELEASE(m_pDepthMap_DSTexDSV[i]);
		SAFE_RELEASE(m_pDepthMap_TexSRV[i]);

		SAFE_DELETE(m_pDepthMap_TexSRV[i]);
	}
	// --------------------------
