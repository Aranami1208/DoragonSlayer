...
namespace {
	...
	const float AttackDistLimit = 15;       // 攻撃接近限界値
	const int   MaxHitPoint = 5000;
...
EnemyBd::EnemyBd()
{
	...
	mesh = ObjectManager::FindGameObject<EnemyManager>()->MeshList("Bd");	  // EnemyManagerのメッシュを参照する
...
SphereCollider EnemyBd::Collider()
{
	// 少し小さめのバウンディングボールとする
	SphereCollider col;
	col.radius = 4.0f;
...
void EnemyBd::updateNormal()
{
	rootNode->Run();	// ビヘイビアツリーの実行
}

void EnemyBd::updateDamage()
{
	state = stFlash;
	flashTimer = MaxFlashTime;
}
void EnemyBd::updateDead()
{
...
void EnemyBd::makeBehaviour()
{
	...
	auto* node6 = node5->AddChild<BehaviourSequence>();
	node5->AddChild <BehaviourAction>(this, actionAttackFlame);
	node6->AddChild<BehaviourAction>(this, actionAttackSelect);
	node6->AddChild<BehaviourAction>(this, actionAttackFireBall);
}
...
BehaviourBase::BtState EnemyBd::actionIsReach(Object3D* objIn)
{
	auto* my = dynamic_cast<EnemyBd *>(objIn);
...
BehaviourBase::BtState EnemyBd::actionIdle(Object3D* objIn)
{
	auto* my = dynamic_cast<EnemyBd *>(objIn);
...
BehaviourBase::BtState EnemyBd::actionWalk(Object3D* objIn)   // ルート／エリアに従って移動
{
	auto* my = dynamic_cast<EnemyBd*>(objIn);
...
BehaviourBase::BtState EnemyBd::actionReach(Object3D* objIn)   // Playerに向かって移動
{
	auto* my = dynamic_cast<EnemyBd*>(objIn);
...
BehaviourBase::BtState EnemyBd::actionAttackFireBall(Object3D* objIn)	 // 火の玉攻撃処理
{
	auto* my = dynamic_cast<EnemyBd*>(objIn);
	...
	if ((int)(my->animator->CurrentFrame()) == 60)    // 攻撃アニメーションが攻撃態勢に入ったとき
	{ 
		// 攻撃
		MATRIX4X4 world = my->mesh->GetFrameMatrices(my->animator, my->transform.matrix(), 9);	 // ボーン番号９から発射
		wm->SpawnMany<WeaponFireBall>(GetPositionVector(world), my->targetPlayer->Collider().center, WeaponBase::eENM);
	}
...
BehaviourBase::BtState EnemyBd::actionAttackFlame(Object3D* objIn)	 // 火炎攻撃処理
{
	auto* my = dynamic_cast<EnemyBd*>(objIn);
	...
	if (((int)(my->animator->CurrentFrame())) > 60 && 
		((int)(my->animator->CurrentFrame())) % 4 == 0)    // 攻撃アニメーション
	{ 
		// 攻撃
		MATRIX4X4 world = my->mesh->GetFrameMatrices(my->animator, my->transform.matrix(), 9); 	 // ボーン番号９から発射
		wm->SpawnMany<WeaponFireBall2>(GetPositionVector(world), my->targetPlayer->Collider().center, WeaponBase::eENM);
	}
	if (my->animator->Finished()) // 攻撃アニメーションが終了？
	{
		my->animator->Play(aRun);
...
BehaviourBase::BtState EnemyBd::actionAttackSelect(Object3D* objIn)	 // 火の玉攻撃と火炎攻撃の選択処理
{
